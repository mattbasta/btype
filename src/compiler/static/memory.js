...
* @return uint (0 or 1) Value of the bit
*/
...
function _malloc_toggle(index, bit) {
    ...
    idxVal = index / 4 | 0;
    // Determine the value to compose in.
    // `1<<x` is 2^x in real math.
    posVal = 1 << index % 4 + bit;
...
* @return uint Pointer to free memory block or null
*/
function _malloc_search(pointer, pairSize, requestedSize, midpoint) {
    ...
    var nextSize = 0;
    var nextHalfSize = 0;
    nextSize = pairSize / 2 | 0;
    nextHalfSize = nextSize / 2 | 0;
    // Will the block not fit in one of the pairs?
    if (nextSize < requestedSize || nextSize < $LOWEST_ORDER$) {
        ...
        if (midpoint === 0) {
            ...
            return _malloc_search(pointer - nextHalfSize | 0, nextSize, requestedSize) | 0;
        } else {
            // If the left side is split or unallocated, go that route.
            temp = _malloc_lookup(pointer - nextHalfSize | 0, 0) | 0;
            if (temp === 1 || _malloc_lookup(pointer - nextSize | 0, 1) | 0 === 0) {
                // Search that side.
            ...
            temp = _malloc_lookup(pointer + nextHalfSize | 0, 0) | 0;
            if (temp === 1 || _malloc_lookup(pointer | 0, 1) | 0 === 0) {
                // Search this side.
                temp = _malloc_search(pointer + nextHalfSize | 0, nextSize, requestedSize, temp) | 0;
                if (temp !== 0) {
                    return temp | 0;
...
* @return uint Pointer to location of allocated memory
*/
function malloc(bytes) {
    ...
    halfHeap = HEAP_SIZE / 2 | 0;
    midpoint = _malloc_lookup(halfHeap, 0);
...
function free(pointer) {
    ...
    while (1) {
        ...
        if (_malloc_lookup(buddy + (size / 2 | 0) | 0, 0) === 1 ||
            _malloc_lookup(buddy, 1)) {
            return;
        }
